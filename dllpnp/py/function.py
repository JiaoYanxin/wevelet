...
def decorate_coefficients(coeffs, pitch_angle, offset, zshift, anglesize):
 ...
 # Calculations for coefficients b
    ...
    for i in range(anglesize):
        ...
        coeffs[i].a[0] = a_orig[0] + a_orig[3] * t + a_orig[9] * t**2
        coeffs[i].a[1] = a_orig[1] + a_orig[5] * t
        coeffs[i].a[2] = a_orig[2] + a_orig[6] * t
        coeffs[i].a[3] = a_orig[3] + 2 * a_orig[9] * t

        coeffs[i].b[0] = b_orig[0] + b_orig[3] * t + b_orig[9] * t**2
        coeffs[i].b[1] = b_orig[1] + b_orig[5] * t
        coeffs[i].b[2] = b_orig[2] + b_orig[6] * t
        coeffs[i].b[3] = b_orig[3] + 2 * b_orig[9] * t
