...
public:
	lwt(vector<T> &signal, liftscheme &lft)
	{
		...
		for (int i = 0; i < N; i++)
		{
			...
			vector<double> filt;
			int len_filt = lenv[2 * i];//2 0 
			int max_pow = lenv[2 * i + 1];//0 2
			...
			if (lft_type == 'd')
			{
				...
				for (int len_dl = 0; len_dl < (int)dl.size(); len_dl++)//遍历dl的每一个元素
				{
					double temp = 0.0;
					for (int lf = 0; lf < len_filt; lf++)//2
					{
...
public:
	lwt2(vector<T> &signal, int rows, int cols, liftscheme &lft)
	{
		...
		colHH = cols_hh;
		level = 1;
		coef_lengths.push_back(rowLL);//0
		coef_lengths.push_back(colLL);//1
		coef_lengths.push_back(rowLH);//2
		coef_lengths.push_back(colLH);//3
	...
	void getDetails(string align, int slevel, vector<T> &det_vec, vector<int> &det_len)
	{
		...
		if (align == "LH" || align == "lh")
		{
			...
			for (int i = 0; i < lev; i++)//如果输出==level 不会执行！
			{
		...
		else if (align == "LL" || align == "ll")
		{
			for (int i = 0; i < lev; i++)
			{
				total = total + sig_vec[0 + i * 6] * sig_vec[1 + i * 6];
			}
			det_vec.assign(A1.begin() + total, A1.begin() + total + sig_vec[0 + lev * 6] * sig_vec[1 + lev * 6]);
			det_len.push_back(sig_vec[0 + lev * 6]);
			det_len.push_back(sig_vec[1 + lev * 6]);
		}
