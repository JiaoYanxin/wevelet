                                       ...
                                       int angleIdx, int my_MPI_thread_id)
{
    ...
    // DEBUG:
    /* if (my_MPI_thread_id ==0)
     {
     printf("Begin to write projection_pad.mrc for %d angleIdx\n",angleIdx);
     write_MRC_image_from_IplImage(projection_pad, "/home/wanxiaohua/exp/3D_phantom/reconstruct/txbr-test/projection_pad.mrc",angleIdx);
     }*/
    ...
    /*
     ...
     {
     printf("Begin to write projection_pad.mrc for %d angleIdx\n",angleIdx);
     write_MRC_image_from_CvMat(Matrix1, "/home/wanxiaohua/exp/3D_phantom/reconstruct/txbr-test/projection_pad.mrc",angleIdx);
     }*/
    ...
    /* // DEBUG:
     ...
     if (my_MPI_thread_id ==0)
     {
     printf("Begin to write projection_pad_mat.mrc for %d angleIdx\n",angleIdx);
     write_MRC_image_from_CvMat(Matrix1, "/home/wanxiaohua/exp/3D_phantom/reconstruct/txbr-test/projection_pad_mat.mrc",angleIdx);
     }*/
...
static void filter_Rweighting_1D_many(float *data, int Nx, int Ny, float radial, float sigma)
{
    ...
    float *bufc = new float[(Nx_final + 2 - Nx_final % 2) * Ny]; // ！！！这里连同下方bufc指针需要改成统一内存！！ 更新：不需要！NVIDIA官方说明里面说cufftw库支持不在GPU显存上的数据

    // 找时间把fftw库改成cufft适配库，注意改成cufft库之后需要相应地使用同一内存分配内存空间
    ...
    // #pragma omp parallel for num_threads(threadNumber)
    for (int i = 0; i < Nx * Ny; i++) // normalization  //这里可以改写成cuda函数
    {
        data[i] /= Nx;
    }
    ...
    delete[] bufc; // 这里需要用cuda的方式释放
}

int ApplyFilterInplace(float nx,float ny,float nz, float *data, int mode)
{
    if (data == NULL)
    {
    ...
    projectionParameter.x = nx;
    projectionParameter.y = ny; // 仅为了适配下方函数
    size_t oneSliceSize = nx * ny;
    ...
    case 0: //
    {
        ...
        for (int angleIdx = 0; angleIdx < nz; angleIdx++)
        {
    ...
    case 2:
    {
        ...
        {
            float *currentSlice = data + oneSliceSize * angleIdx;
            filter_Rweighting_1D_many(currentSlice, nx, ny, weighting_radial, weighting_sigma);
        }
    };
    break;
    default:
