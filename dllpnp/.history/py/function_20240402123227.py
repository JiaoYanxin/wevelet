...
def decorate_coefficients(coeffs, pitch_angle, offset, zshift, anglesize):
    ...
    # First loop: Regular coefficient calculations
    for i in range(anglesize):
        a_orig, b_orig = coeffs[i].a.copy(), coeffs[i].b.copy()
        coeffs[i].a[0] = a_orig[0];
		coeffs[i].a[1] = (a_orig[2] * sa * sb + a[3] * ca * sb + a[1] * cb)
		coeffs[i].a[2] = (a_orig[2] * ca - a[3] * sa);						//*y
		coeffs[i].a[3] = (a_orig[3] * ca * cb + a[2] * cb * sa - a[1] * sb); //*z
		coeffs[i].a[4] = (a_orig[4] * ca * cb - a[5] * cb * sa - a[6] * sa2 * sb -
		...
		coeffs[i].b[8] = (b[8] * ca2 + b[9] * sa2 - b[6] * ca * sa);		 //*y^
