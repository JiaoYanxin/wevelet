...
def get_header(content):
    ...
    return header, start, dsize


def load_data(header, content):
    if header.mode == 0:
        dtype = np.int8
    elif header.mode == 1:
        dtype = np.int16
    ...
    array = array[:header.ny * header.nx]
    # reshape the array
    # , order='F')
    array = np.reshape(array, (header.ny, header.nx))
    # if header.nz == 1:
    #     array = array[0]
...
def make_header(shape, cella, cellb, mz=1, dtype=np.float32, order=(1, 2, 3), dmin=0, dmax=-1, dmean=-2, rms=-1, exthd_size=0, ispg=0):
    mode = get_mode(dtype)
