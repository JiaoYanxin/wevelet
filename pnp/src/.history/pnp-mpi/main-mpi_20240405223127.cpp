...
void InitializeHeader(MRCheader &header)
{
...
void ATbmuKT_lwt(const float *atb, float *atb_lt, const float *uk, const float *dk, int width, int length, int height, float mu, const std::vector<int> &dimvec, liftscheme &lft, int J)
{
	size_t volsize = width * length * height;
	std::vector<float> vectors[8];
	for (int i = 0; i < 8; i++)
	{
		int size = dimvec[0 + i * 3] * dimvec[1 + i * 3] * dimvec[2 + i * 3];
	...
	for (int i = 0; i < 8; i++)
	{
		...
		{
			vectors[i].push_back(uk[index] - dk[index]);
			index++;
		}
	}

	ilwt3<float> ilift3(vectors[0], vectors[1], vectors[2], vectors[3], vectors[4], vectors[5], vectors[6], vectors[7], dimvec, lft, J);
	...
	// 	// 准备文件头
	// 	MRCheader outheader;
	// 	InitializeHeader(outheader);
	// 	outheader.nx = dimvec[0];
	// 	outheader.ny = dimvec[1];
	// 	outheader.nz = dimvec[2];

	// 	// 使用max_element找到最大元素的迭代器
	// 	auto max_it = std::max_element( vectors[7].begin(), vectors[7].end());
	// 	auto min_it = std::min_element(vectors[7].begin(), vectors[7].end());
	// 	outheader.amin = *min_it;
	// 	outheader.amax = *max_it;
	// 	outheader.nlabl = 0;
	// 	printf("  outheader.amin ,%d\n", outheader.amin);
	// 	printf("  outheader.amax ,%d\n", outheader.amax);
	// 	// 打开文件
	// 	std::ofstream file("output.mrc");
	// 	if (!file)
	// 	{
	// 		std::cerr << "无法创建文件\n";
	// 	}
	// 	// 写入文件头
	// 	file.write(reinterpret_cast<char *>(&outheader), sizeof(MRCheader));
	// 	// 写入图像数据
	// 	file.write(reinterpret_cast<char *>(vectors[7].data()), vectors[7].size() * sizeof(float));
	// 	file.close();
	// }
	for (int i = 0; i < width * length * height; i++)
	{
		atb_lt[i] = atb[i] + mu * oup[i];
	}
}
...
//先对x进行 处理BM3D去噪
void UpdateU(MrcStackM &projs, const Volume &vol, float *u_k, float *d_k, float eta, float lamb, liftscheme &lft, const std::vector<int> &l_length)
{
	size_t volsize = vol.width * vol.length * vol.height;
	std::vector<float> v_vol;
	v_vol.reserve(volsize); // 和原图一样大 为了放原图
	v_vol.assign(vol.data, vol.data + volsize);
	lwt3<float> lift3(v_vol, vol.width, vol.length, vol.height, lft);
	...
	for (int i = 0; i < 8; i++)
	{
		...
		for (int z = 0; z < l_length[2 + i * 3]; z++) // 每一层
		{
			cv::Mat slice(l_length[0 + i * 3], l_length[1 + i * 3], CV_32F, &temp[z * l_length[0 + i * 3] * l_length[1 + i * 3]]);
			// 只接受0-255的输入
			slice.convertTo(slice, CV_8U, scale, offset);
			cv::Mat sliceFiltered(l_length[0 + i * 3], l_length[1 + i * 3], CV_8U);
			...
			memcpy(&u_k[num + z * l_length[0 + i * 3] * l_length[1 + i * 3]], sliceFiltered.data, l_length[0 + i * 3] * l_length[1 + i * 3] * sizeof(float));
		}
		num += size;
		free(temp);
	}
...
// 	for (int i = 0; i < 8; i++)
// 	{
// 		int lwtsize = l_length[0 + i * 3] * l_length[1 + i * 3] * l_length[2 + i * 3];
// 		vectors[i].reserve(lwtsize);
// 	}
// 	lift3.getCoef(vectors[0], vectors[1], vectors[2], vectors[3], vectors[4], vectors[5], vectors[6], vectors[7]);
...
// 	{
// 		int lwtsize = l_length[0 + i * 3] * l_length[1 + i * 3] * l_length[2 + i * 3];
// 		float *temp = (float *)malloc(sizeof(float) * lwtsize);

// 		for (int j = 0; j < lwtsize; j++)
// 		{
// 			temp[j] = vectors[i][j] + d_k[index];
// 			index++;
// 		}
...
// 		size[0] = l_length[0 + i * 3]; // X尺寸
// 		size[1] = l_length[1 + i * 3]; // Y尺寸
// 		size[2] = l_length[2 + i * 3]; // Z尺寸
// 									   // 图像原点
...
// 		image->Allocate();
// 		// 填充ITK图像：将一维数组中的数据复制到ITK图像中。需要将一维索引转换为三维索引。
// 		for (unsigned int z = 0; z < l_length[2 + i * 3]; ++z)
// 		{
// 			for (unsigned int y = 0; y < l_length[1 + i * 3]; ++y)
// 			{
// 				for (unsigned int x = 0; x < l_length[0 + i * 3]; ++x)
// 				{
...
// 					image->SetPixel(pixelIndex, temp[z * l_length[1 + i * 3] * l_length[0 + i * 3] + y * l_length[0 + i * 3] + x]);
// 				}
// 			}
// 		}
// 		// 应用高斯滤波器
...
// 		float sigma = eta / lamb;
// 		if (i == 0)
// 		{
// 			sigma = sigma / 4;
// 		}
// 		filter->SetInput(image);
// 		filter->SetSigma(sigma); // 根据需要设置Sigma值 eta/lamb?
// 		// 更新滤波器并获取结果：
// 		filter->Update();
...
// 					// 获取滤波后的像素值
// 					PixelType pixelValue = filteredImage->GetPixel(pixelIndex);

// 					// 将像素值复制回一维数组
// 					u_k[num + z * l_length[1 + i * 3] * l_length[0 + i * 3] + y * l_length[0 + i * 3] + x] = pixelValue;
// 				}
// 			}
// 		}
// 		free(temp);
...
void UpdateD(const Volume &vol, const float *u_k, float *d_k, size_t volsize, liftscheme &lft, const std::vector<int> &l_length)
{
	std::vector<float> v_vol;
	v_vol.reserve(vol.width * vol.length * vol.height);
	v_vol.assign(vol.data, vol.data + vol.width * vol.length * vol.height);
	lwt3<float> lift3(v_vol, vol.width, vol.length, vol.height, lft);
	std::vector<float> vectors[8];
	for (int i = 0; i < 8; i++)
	{
	...
	for (int i = 0; i < 8; i++)
	{
		...
		for (int j = 0; j < size; j++)
		{
			d_k[index] = d_k[index] + vectors[i][j] - u_k[index];
			index++;
		}
	}
}
...
// 可以用稀疏矩阵存放A：数据：4*体的大小， 但是切分的话其实不用太考虑大小
{
	...
	for (int i = 0; i < maxOutIter; i++)
	{
		...
		ATbmuKT_lwt(htb, htb_lt, u_k, d_k, vol.width, vol.length, vol.height, lamb, v_length, blift, J); // 在循环内 每次更新ud
		// printf("ATAmuLTL");
		// vol不变 x0ATA(vol)+mu*LTL(vol,L,LTL)
		{
			float *atax = (float *)malloc(sizeof(float) * volsize);
