...
void L(const Volume &vol, float *L)
{
	size_t volsize = vol.width * vol.length * vol.height;
...
// ATb+mu*K^T(u1-d1,u2-d2,u3-d3)
// 结果在htb_lt
// 先把u-d相减的结果放进vector中 再进行三维idwt 再和ATb相加
void ATbmuKT_lwt(float *atb, float *atb_lt, float *uk, float *dk, int width, int length, int height, float mu, std::vector<int> &dimvec, liftscheme &lft, int J)
{
	// size_t size = width * length * height / 8;
	size_t size = dimvec[0] * dimvec[1] * dimvec[2]; // 和LLL一样大
	std::vector<float> v;
	// 预先为vector分配空间
	v.reserve(size);
	for (int i = 0; i < size; i++)
	{
		v.push_back(uk[i] - dk[i]);
	}
	// //	// V是LLL 其余部分全填0
	std::vector<float> a(v.size(), 0);
	ilwt3<float> ilift3(v, dimvec, lft, J);
	// std::vector<float> aLLL, aLLH, aLHL, aLHH, aHLL, aHLH, aHHL, aHHH;
	// lift3.getCoef(aLLL, aLLH, aLHL, aLHH, aHLL, aHLH, aHHL, aHHH);
	// ilwt3<float> ilift3(v, aLLH, aLHL, aLHH, aHLL, aHLH, aHHL, a, dimvec, lft);

	std::vector<float> oup;
	oup.reserve(width * length * height); // 和原图一样大
	ilift3.getSignal(oup);
	for (int i = 0; i < width * length * height; i++)
	{
		atb_lt[i] = atb[i] + oup[i];
	}
}
...
// 结尾取0 DX=x-xn+1
void Dx(const float *vol, float *ltl, int width, int length, int height, int mu)
{
	size_t volsize = width * length * height;
	size_t n;
	...
	for (int z = 0; z < height; z++)
	{
		const float *drez = vol + z * (size_t)width * length; // 体的值
		float *drez_lx = ltl + z * (size_t)width * length;

		for (int y = 0; y < length; y++)
		{
			const float *drey = drez + y * width; // 值
			float *drey_lx = drez_lx + y * width;

			for (int x = 0; x < width; x++)
			{
				const float *drex = drey + x;
				float *L_x = drey_lx + x;
				// L
				if (x + 1 == width)
				{
...
// 结尾取0 DX=x-xn+1
void DTx(const float *vol, float *ltl, int width, int length, int height, int mu)
{
	size_t volsize = width * length * height;
	size_t n;
...
void Dy(const float *vol, float *ltl, int width, int length, int height, int mu)
{
	size_t volsize = width * length * height;
	size_t n;
	...
	for (int z = 0; z < height; z++)
	{
		...
		for (int y = 0; y < length; y++)
		{
			...
			for (int x = 0; x < width; x++)
			{
				...
				{
					*dreL += mu * (*drex - *(drex + width));
				}
			}
		}
	}
}
void DTy(const float *vol, float *ltl, int width, int length, int height, int mu)
{
	size_t volsize = width * length * height;
	size_t n;
...
void Dz(const float *vol, float *ltl, int width, int length, int height, int mu)
{
	size_t volsize = width * length * height;
	size_t n;
	...
	for (int z = 0; z < height; z++)
	{
		...
		for (int y = 0; y < length; y++)
		{
			...
			for (int x = 0; x < width; x++)
			{
				...
				{
					*dreL += mu * (*drex - *(drex + width * length)); // zn-zn+1
				}
			}
		}
	}
}
void DTz(const float *vol, float *ltl, int width, int length, int height, int mu)
{
	size_t volsize = width * length * height;
	size_t n;
	...
	for (int z = 0; z < height; z++)
	{
		...
		for (int y = 0; y < length; y++)
		{
			...
			for (int x = 0; x < width; x++)
			{
				...
				if (z > 0)
				{
					*dreL += mu * (*drex - *(drex - width * length));
				}
				else
				{
					*dreL += 0;
...
// (ATA)x+(DxTDx+DyTDy+DzTDz)X
void ATAmuDTD(const float *vol, const float *ata, float *ltl, float mu1, float mu2, float mu3, int width, int length, int height)
{
	size_t volsize = width * length * height;
	memcpy(ltl, ata, volsize * sizeof(float));

	float *temp = (float *)malloc(sizeof(float) * volsize);

	memset(temp, 0, sizeof(float) * volsize);
	// temp=1*(Dx)X
	Dx(vol, temp, width, length, height, 1); // 这里只需要DT乘就行
	// ltl=ata+mu1*(DxTDx)X
	DTx(temp, ltl, width, length, height, mu1);

	memset(temp, 0, sizeof(float) * volsize);
	// temp=Dy
	Dy(vol, temp, width, length, height, 1);
	DTy(temp, ltl, width, length, height, mu2);

	memset(temp, 0, sizeof(float) * volsize);
	// temp=Dz
	Dz(vol, temp, width, length, height, 1);
	DTz(temp, ltl, width, length, height, mu3);
...
void applycg(Volume &vol, float *atax0, float *ATb, int numberIteration, float mu1, float mu2, float mu3, const Point3DF &origin, Coeff coeffv[], int proj, MrcStackM &projs, float *htb)
{
	// 初始化
	size_t volsize = vol.length * vol.width * vol.height;
	float *r0, *p0;
	...
	{ // 最大迭代次数
		...
		memset(h0, 0, sizeof(float) * volsize); // h0是作为Apk 是一个中间变量
		{
			...
			ATAmuDTD(p0, atax, h0, mu1, mu2, mu3, vol.width, vol.length, vol.height);
			free(atax);
		}
		// 计算alpha
		...
		}
		{
			...
			ATAmuDTD(vol.data, atax, ax, mu1, mu2, mu3, vol.width, vol.length, vol.height);
			free(atax);
		}
		for (int k = 0; k < volsize; k++)
		{
...
// 先对x进行处理
void UpdateU(const Volume &vol, float *u_k, const float *d_k, float eta, int label)
{
	size_t volsize = vol.length * vol.width * vol.height;
	float *temp = (float *)malloc(sizeof(float) * volsize);
	memset(temp, 0, sizeof(float) * volsize);
	...
	if (label == 1)
	{
		Dx(vol.data, temp, vol.width, vol.length, vol.height, 1);
	}
	else if (label == 2)
	{
		Dy(vol.data, temp, vol.width, vol.length, vol.height, 1);
	}
	else if (label == 3)
	{
		Dz(vol.data, temp, vol.width, vol.length, vol.height, 1);
	}
	else
	{
		printf("label error!");
	...
	for (int z = 0; z < vol.height; z++)
	{
		cv::Mat slice(vol.width, vol.length, CV_32F, &temp[z * vol.width * vol.length]);
		slice.convertTo(slice, CV_8U, scale, offset); // 只接受0-255的输入

		cv::Mat sliceFiltered(vol.width, vol.length, CV_8U);

		cv::xphoto::bm3dDenoising(slice, sliceFiltered, eta); // eta/lamb是滤波强度
		// 转换回去
		sliceFiltered.convertTo(sliceFiltered, CV_32F, inv_scale, inv_offset);
		memcpy(&u_k[z * vol.width * vol.length], sliceFiltered.data, vol.width * vol.length * sizeof(float));
	}
	// std::ofstream file("filterd.txt");
	// if (file)
	// {
	// 	for (int i = 0; i < vol.width * vol.length * vol.height / 8; i++)
	// 	{
	// 		file << u_k[i] << std::endl; // 或者根据需要调整格式
	// 	}
	// 	file.close(); // 显式关闭文件流
	// }
	// else
...
// 		slice.convertTo(slice, CV_8U, 255.0);
...
// uk+1= bm3d(LX+U)
void UpdateU_admmtest(const Volume &vol, float *u_k, const MrcStackM &projs, float eta, float lamb)
{
	size_t size = vol.length * vol.width * vol.height;
	// 临时变量
	float *temp = (float *)malloc(sizeof(float) * size);
	L(vol, temp);				   // temp=LX
	for (int i = 0; i < size; i++) // lx+u
	{
		temp[i] = temp[i] + u_k[i];
	}
	...
	for (int z = 0; z < vol.height; z++)
	{
		...
		cv::Mat sliceFiltered(projs.X(), projs.Y(), CV_8U);

		cv::fastNlMeansDenoising(slice, sliceFiltered, eta); // eta/lamb是滤波强度
		// 转换回去
		sliceFiltered.convertTo(sliceFiltered, CV_32F, inv_scale, inv_offset);
		memcpy(&u_k[z * projs.X() * projs.Y()], sliceFiltered.data, projs.X() * projs.Y() * sizeof(float));
	}
...
// dk=dk-1+LX-uk
void UpdateD_admmtest(const Volume &vol, const float *u_k, float *d_k, size_t volsize)
{
...
void UpdateD(const Volume &vol, const float *u_k, float *d_k, size_t volsize)
{
	...
	for (int k = 0; k < volsize; k++)
	{
		d_k[k + volsize] = d_k[k + volsize] + temp[k] - u_k[k + volsize];
	}
	...
	for (int k = 0; k < volsize; k++)
	{
		d_k[k + 2 * volsize] = d_k[k + 2 * volsize] + temp[k] - u_k[k + 2 * volsize];
	}
}
...
// 可以用稀疏矩阵存放A：数据：4*体的大小， 但是切分的话其实不用太考虑大小
{
	...
	// 	applycg(vol, x0, htb_lt, numberIteration, mu, origin, coeffv, projs.Z(), projs, htb); // vol已经更新->x_k 也会作为下一次cg迭代的初始值
	// 	free(x0);
	// 	free(htb_lt);
	// 	// 更新u、d
	// 	L(vol, u_k);
	// 	float soft = 10;
	// 	UpdateU_admmtest(vol, u_k, projs, mu, 0.8);
	// 	// 更新d
	// 	UpdateD_admmtest(vol, u_k, d_k, volsize);
	// }
	// free(u_k);
	// free(d_k);
	// free(htb);
...
// 可以用稀疏矩阵存放A：数据：4*体的大小， 但是切分的话其实不用太考虑大小
{
	...
	// 先计算一次x的小波变换 为了得到length矩阵
	std::vector<float> v_vol;
	v_vol.reserve(volsize);
	v_vol.assign(vol.data, vol.data + volsize);

	string name = "haar";
	int J = 4;
	liftscheme blift(name);
	std::vector<int> v_length;
	lwt3<float> lift3(v_vol, vol.width, vol.length, vol.height, blift, J);
	...
	for (int i = 0; i < maxOutIter; i++)
	{
		...
		memset(x0, 0, sizeof(float) * volsize);
		// ATb+muKT(u-d)
		// ATbmuIT(htb, htb_lt, u_k, d_k, vol.width, vol.length, vol.height, lamb);
		{
			float *temp = (float *)malloc(sizeof(float) * volsize);
			for (int num = 0; num < volsize; num++)
			{
				temp[num] = u_k[num] + d_k[num]; // u1+d1
				htb_lt[num] = htb[num];
			}
			// htb_lt=htb+mu1*(DxT)(u1+d1)
			DTx(temp, htb_lt, vol.width, vol.length, vol.height, mu1);

			memset(temp, 0, sizeof(float) * volsize);
			for (int num = 0; num < volsize; num++)
			{
				temp[num] = u_k[num + volsize] + d_k[num + volsize]; // u2+d2
			}
			DTy(temp, htb_lt, vol.width, vol.length, vol.height, mu2);

			memset(temp, 0, sizeof(float) * volsize);
			for (int num = 0; num < volsize; num++)
			{
				temp[num] = u_k[num + 2 * volsize] + d_k[num + 2 * volsize]; // u3+d3
			}
			DTz(temp, htb_lt, vol.width, vol.length, vol.height, mu3);
		...
		//  vol不变 x0ATA(vol)+mu*LTL(vol,L,LTL)
		{
			...
			ATAmuDTD(vol.data, atax, x0, mu1, mu2, mu3, vol.width, vol.length, vol.height);
			free(atax);
		}
		// vol就是ATAx0
		printf("**  Conjugate Gradient Method  **\n");
		applycg(vol, x0, htb_lt, numberIteration, mu1, mu2, mu3, origin, coeffv, projs.Z(), projs, htb); // vol已经更新->x_k 也会作为下一次cg迭代的初始值
		free(x0);
		free(htb_lt);
		// 更新u、d
		// 更新u 使用去噪算子NLM
		{ // 更新u1
			...
			// UpdateU(vol, u_k, d_k, eta, 1);
			UpdateU(vol, &u_k[volsize], &d_k[volsize], eta, 2);
			UpdateU(vol, &u_k[2 * volsize], &d_k[2 * volsize], eta, 3);
		}
		// 更新d
...
int ATOM(options &opt, int myid, int procs)
{
	...
	int zrem = mrcvol.Z() % procs;		 // 得到的余数
	int volz;							 // the start slice of reproject per process
	int baseHeight = mrcvol.Z() / procs; // 每个线程本应该得到的数
	int remainder = 0;
	...
	if (zrem != 0) // 如果有剩下的
	{
		if (baseHeight % 2 != 0)
		{
			...
			remainder = mrcvol.Z() - baseHeight * procs; // 现在剩下的余数
		}
		else
		{
	...
	else if (opt.method == "PnP")
	{
		...
		else
		{
			...
			init.ReadHeader();
			{
				int j = 0;
				for (j = 0; vol.z + j + 5 < vol.z + vol.height; j += 5)
					init.ReadBlock(vol.z + j, vol.z + j + 5, 'z', (vol.data + (size_t)vol.width * vol.length * j));
				init.ReadBlock(vol.z + j, vol.z + vol.height, 'z', (vol.data + (size_t)vol.width * vol.length * j));
				// printf("2");
				//  projs.ReadBlock(vol.z, vol.z + vol.height, 'z', (vol.data));
			}
			init.Close();
