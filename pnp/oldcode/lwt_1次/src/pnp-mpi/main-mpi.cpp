...
void ATbmuKT_lwt(float *atb, float *atb_lt, float *uk, float *dk, int width, int length, int height, float mu, std::vector<int> &dimvec, liftscheme &lft)
{
	...
	// //	// V是LLL 其余部分全填0
	 ...
	 ilwt3<float> ilift3(v, a, a, a, a, a, a, a, dimvec, lft);
...
// 先对x进行处理
void UpdateU(const Volume &vol, float *u_k, float *d_k, float eta, float lamb, liftscheme &lft, std::vector<int> &l_length)
{
	std::vector<float> sig;
	sig.reserve(vol.width * vol.length * vol.height); // 和原图一样大 为了放原图
	sig.assign(vol.data, vol.data + vol.width * vol.length * vol.height);
	lwt3<float> lift3(sig, vol.width, vol.length, vol.height, lft);

	int size = l_length[0] * l_length[1] * l_length[2];
	...
	for (int z = 0; z < l_length[2]; z++)
	{
		cv::Mat slice(l_length[0], l_length[1], CV_32F, &temp[z * l_length[0] * l_length[1]]);
		// 只接受0-255的输入

		slice.convertTo(slice, CV_8U, scale, offset);
		cv::Mat sliceFiltered(l_length[0], l_length[1], CV_8U);
		...
		memcpy(&u_k[z * l_length[0] * l_length[1]], sliceFiltered.data, l_length[0] * l_length[1] * sizeof(float));
	}
...
void UpdateD(const Volume &vol, const float *u_k, float *d_k, size_t volsize, liftscheme &lft, std::vector<int> &l_length)
{
	std::vector<float> sig;
	sig.reserve(vol.width * vol.length * vol.height);
	sig.assign(vol.data, vol.data + vol.width * vol.length * vol.height);
	lwt3<float> lift3(sig, vol.width, vol.length, vol.height, lft);
	int size = l_length[0] * l_length[1] * l_length[2];
	std::vector<float> aLLL;
...
// 可以用稀疏矩阵存放A：数据：4*体的大小， 但是切分的话其实不用太考虑大小
{
	...
	lwt3<float> lift3(v_vol, vol.width, vol.length, vol.height, blift);
	lift3.getDim(v_length);

	float *u_k = (float *)malloc(sizeof(float) * v_length[0] * v_length[1] * v_length[2]); // 如果不够
	float *d_k = (float *)malloc(sizeof(float) * v_length[0] * v_length[1] * v_length[2]);
	memset(u_k, 0, sizeof(float) * v_length[0] * v_length[1] * v_length[2]);
	memset(d_k, 0, sizeof(float) * v_length[0] * v_length[1] * v_length[2]);
	...
	for (int i = 0; i < maxOutIter; i++)
	{
		...
		ATbmuKT_lwt(htb, htb_lt, u_k, d_k, vol.width, vol.length, vol.height, lamb, v_length, blift); // 在循环内 每次更新ud
		// printf("ATAmuLTL");
		// vol不变 x0ATA(vol)+mu*LTL(vol,L,LTL)
		{
			float *atax = (float *)malloc(sizeof(float) * volsize);
		...
		// 更新u、d
		// 更新u 使用去噪算子NLM
		UpdateU(vol, u_k, d_k, eta, lamb, blift, v_length);
		// 更新d
