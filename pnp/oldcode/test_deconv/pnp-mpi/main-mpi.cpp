...
// BM3D去噪
void UpdateU(Volume &vol, float *d_k, float *u_k, const MrcStackM &projs, float eta, float lamb)
{
	// 计算缩放因子
	size_t size = vol.width * vol.length * vol.height;
...
// 可以用稀疏矩阵存放A：数据：4*体的大小， 但是切分的话其实不用太考虑大小
{
	...
	for (int i = 0; i < maxOutIter; i++)
	{
		...
		// 更新u、d
		// 更新u 使用去噪算子NLM
		UpdateU(vol, d_k, u_k, projs, eta, lamb);
		// 更新d
