...
int ApplyFilterInplace(MrcStackM &inputProjection, float *data, int mode)
{
    if (data == NULL)
    {
    ...
    projectionParameter.y = inputProjection.header.ny; // 仅为了适配下方函数
    ...
    case 2:
    {
        ...
        {
            float *currentSlice = data + oneSliceSize * angleIdx;
            filter_Rweighting_1D_many(currentSlice, inputProjection.header.nx, inputProjection.header.ny, weighting_radial, weighting_sigma);
        }
    };
    break;
    default:
